...
datalabels = ['coast', 'sandiego', 'pavia']
...
def train(use_ALDR):
    net = UADNet(R, B, K, drop_out=0.2, use_ALDR=use_ALDR).cuda()
    ...
    for epoch in range(1000):
        for i, (y, y_pca)in enumerate(train_y):
            ...
            LossUnmixing = Reconstructionloss(y1, y_hat1, lambda_mse) + 0.1 * LossTVM
            LossUnmixing = 0.8 * Reconstructionloss(y_normal, yhat_normal, lambda_mse) + 0.2 * Reconstructionloss(y_abnormal, yhat_abnormal, lambda_mse) + 0.1 * LossTVM
            LossClustering = lambda_energy * LossEnergy + lambda_cov * LossCov
            Loss = LossUnmixing + LossClustering
    ...
    torch.save(net.state_dict(), './weight/weights.pt')
...
print('Start test!')
net = UADNet(R, B, K, drop_out=0.2, use_ALDR=use_ALDR).cuda()
net.load_state_dict(torch.load('./weight/weights.pt'))
net.eval()
with torch.no_grad():
...
A_est, _ = norm_abundance(A_est, A_est)
M_est, M_vca = norm_endmember(M_est, M_vca)
...
sio.savemat('./result/' + 'results_' + datalabel + '.mat',{'A_est':A_est,
                                                   'M_est':M_est,
                                                   'Map':Map_gt,
                                                   'ADmap':ADmap,
                                                   })
...
plt.axis("off")
plt.show()
plot_abundance(A_est, A_est)
plot_endmember(M_est, M_vca)
